document.addEventListener('DOMContentLoaded', function() {
    console.log("modal_horario.js cargado ");

    const modal = document.getElementById('deleteModalHorario');
    const closeModal = document.querySelector('.close-modal');
    const cancelBtn = document.querySelector('.btn-cancel');
    const confirmDelete = document.getElementById('confirmDeleteHorario');
    const horarioName = document.getElementById('horarioName');
    ...
    function closeModalFunc() {
        ...
        horarioName.textContent = '';
    }

    document.querySelectorAll('.delete-btn').forEach(button => {
        button.addEventListener('click', function() {
            const name = this.getAttribute('data-horario-name') || '—';
            currentDeleteUrl = this.getAttribute('data-delete-url');
            currentDeleteButton = this;

            horarioName.textContent = `"${name}"`;
    ...
    confirmDelete.addEventListener('click', function(e) {
        ...
        .catch(error => {
            ...
            showMessage('Error al eliminar el horario', 'error');
            confirmDelete.textContent = 'Sí, Eliminar';
            confirmDelete.disabled = false;
        });
    });
