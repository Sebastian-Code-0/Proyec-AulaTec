document.addEventListener('DOMContentLoaded', function() {
    const modal = document.getElementById('deleteModalCalificacion');
    const closeModal = document.querySelector('.close-modal');
    const cancelBtn = document.querySelector('.btn-cancel');
    const confirmDelete = document.getElementById('confirmDeleteCalificacion');
    const calificacionName = document.getElementById('calificacionName');
    ...
    function closeModalFunc() {
        ...
        if (calificacionName) calificacionName.textContent = '';
    }

    document.querySelectorAll('.delete-btn').forEach(button => {
        button.addEventListener('click', function() {
            const name = this.getAttribute('data-calificacion-name') || '—';
            currentDeleteUrl = this.getAttribute('data-delete-url');
            currentDeleteButton = this;
            if (calificacionName) calificacionName.textContent = '"' + name + '"';
    ...
    confirmDelete.addEventListener('click', function(e) {
        ...
        .then(response => {
            if (response.ok) {
                ...
                closeModalFunc();
                showMessage('Calificación eliminada correctamente', 'success');
            } else {
