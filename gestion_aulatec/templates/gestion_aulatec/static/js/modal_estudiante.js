document.addEventListener('DOMContentLoaded', function() {
    console.log("modal_estudiante.js cargado ");

    const modal = document.getElementById('deleteModal');
    const closeModal = document.querySelector('.close-modal');
    const cancelBtn = document.querySelector('.btn-cancel');
    const confirmDelete = document.getElementById('confirmDelete');
    const estudianteName = document.getElementById('estudianteName');
    ...
    function closeModalFunc() {
        ...
        estudianteName.textContent = '';
    }

    // Abrir modal en cada botón
    document.querySelectorAll('.delete-btn').forEach(button => {
        button.addEventListener('click', function() {
            const name = this.getAttribute('data-estudiante-name') || '—';
            currentDeleteUrl = this.getAttribute('data-delete-url');
            currentDeleteButton = this;

            estudianteName.textContent = `"${name}"`;
    ...
    confirmDelete.addEventListener('click', function(e) {
        ...
        .then(response => {
            if (response.ok) {
                // Animación y eliminación de fila
                if (currentDeleteButton) {
                    ...
                    if (row) {
                        ...
                        setTimeout(() => {
                            row.remove();
                            // Si la tabla quedó vacía, recargamos para mostrar mensaje vacío, etc.
